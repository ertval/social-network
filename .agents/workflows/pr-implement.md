...
3. **Formulate Test Plan**:
   - Detail the Go table-driven tests and store/integration test setups (using independent SQLite database instances).
...
1. **TDD Loop**:
   - **RED**: Write failing unit or integration tests (`commands/*_test.go`, `queries/*_test.go`, or `store/*_test.go`). Run `go test` to verify they fail.
   - **GREEN**: Write the minimal code to satisfy the tests. Ensure database stores accept `platform/database.DB` (D4).
   - **REFACTOR**: Clean up code and test structures. Verify performance and run `make ci` or equivalent gates.
2. **SQLite Rules**:
...
3. If any gates fail, output error locations and revert to Phase 3.
