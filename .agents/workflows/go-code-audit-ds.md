---
name: go-code-audit-ds
description: |
  DeepS multi-phase Go codebase audit workflow. Orchestrator-thin,
  specialist-deep analysis with deterministic grounding (golangci-lint,
  govulncheck, go vet, static analysis), layered cognitive review across
  architecture/security/performance, independent Judge/Critic verification in
  a fresh context (Chain-of-Verification, hallucination gates, separated
  verifier, Socratic challenge), and synthesis to
  docs/audit/codebase_audit_report.md. Implements 2026 best practices from
  AEGIS, Sequoia, Code Broker, CoVe, and cross-model adversarial review
  research.
---
...
2. **Determinism before cognition** — run tools (linters, vulncheck, vet) before making cognitive claims. Record raw output to `.audit/phase1-baseline.md`.
...
### Step 1.6 — Nil analysis (if available)
If `nilaway` or `gosimple` MCP is available, run nil-safety analysis to detect potential nil pointer dereferences.

### Step 1.7 — Record baseline
...
6. **CSRF / CORS** — verify CORS configuration restricts allowed origins; check for CSRF protections on state-changing requests.
...
5. **Lock contention** — inspect hot paths for mutex contention, especially around shared state in WebSocket hubs or notification broadcasters.

### Layer E — Dependency & Supply Chain

1. **Vulnerable deps** — cross-reference Phase 1.2 (govulncheck) output with go.mod. Flag any unpatched CVE.
2. **Deprecated modules** — check go.sum for modules that are archived or have known end-of-life notices.
