...
1. **Backend Checks (Go):**
   - Run module verification: `rtk make ci-mod`
   - Run formatting check: `rtk make check-format` (run `rtk make format` first to auto-fix if necessary, but report if modifications were staged)
   - Run linting: `rtk make lint` (staticcheck and golangci-lint)
   - Run tests: `rtk make test` (verify tests pass with race detector enabled)
2. **Frontend Checks (TypeScript/Next.js):**
   - Run frontend gate: `rtk make fe-ci` (Biome lint + format:check + tsc + Vitest)
...
*[Detail results from the linters and tests. If failure, print the exact failing commands and stderr output]*
- **Go Mod Tidy:** `PASS` / `FAIL`
- **Go Format & Imports:** `PASS` / `FAIL`
- **Go Lints (staticcheck/golangci-lint):** `PASS` / `FAIL`
- **Go Unit Tests:** `PASS` / `FAIL`
- **Frontend Lint (Biome):** `PASS` / `FAIL`
- **Frontend Format (Biome):** `PASS` / `FAIL`
...
* **File & Line Range:** `[filename.go#L12-L24](file:///relative/path/to/file#L12-L24)` *(Ensure standard markdown relative links)*
...
* `[path/to/file1.go](file:///relative/path/to/file1.go)`
* `[path/to/file2.go](file:///relative/path/to/file2.go)`
...
- [ ] **Deterministic fixes:**
  ...
  - [ ] Fix compiler/linter warnings in `[file](file:///...)` by applying...
- [ ] **Architecture fixes:**
  - [ ] Remove `internal/store/...` import from `internal/commands/...` as per D5 rules.
- [ ] **Security fixes:**
  - [ ] Replace query string concatenation with parameterized SQL parameters in `[store/sqlite.go](file:///...)`.
```
