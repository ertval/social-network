...
- `.editorconfig`, `eslint.config.*`, `eslint.config.mjs`, `prettier.config.*`, `tsconfig.json` (machine-enforced standards — note them but don't re-check what tooling already checks)
