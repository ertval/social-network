...
func (r *Repo) CastVote(ctx context.Context, userID string, target vote.Target, reactionType int) error {
	...
	if target.CommentID != nil {
		...
		checkQuery := `SELECT reaction_type FROM votes WHERE user_id = $1 AND comment_id = $2 AND topic_id IS NULL`
		...
		if err == nil && existingReaction.Valid && int(existingReaction.Int32) == reactionType {
			...
			deleteQuery := `DELETE FROM votes WHERE user_id = $1 AND comment_id = $2 AND topic_id IS NULL`
		...
		// Check if vote exists with same reaction type
		var existingReaction sql.NullInt32
		checkQuery := `SELECT reaction_type FROM votes WHERE user_id = $1 AND topic_id = $2 AND comment_id IS NULL`
		err := r.DB.QueryRowContext(ctx, checkQuery, userID, *target.TopicID).Scan(&existingReaction)

		if err == nil && existingReaction.Valid && int(existingReaction.Int32) == reactionType {
			// Same vote - delete it (toggle off)
			deleteQuery := `DELETE FROM votes WHERE user_id = $1 AND topic_id = $2 AND comment_id IS NULL`
...
func (r *Repo) DeleteVote(ctx context.Context, userID string, topicID *int, commentID *int) error {
	...
	builder.WriteString("DELETE FROM votes WHERE user_id = $1")
	args = append(args, userID)

	paramPos := 1
	if commentID != nil {
		paramPos++
		builder.WriteString(fmt.Sprintf(" AND comment_id = $%d AND topic_id IS NULL", paramPos))
		args = append(args, *commentID)
	} else {
		paramPos++
		builder.WriteString(fmt.Sprintf(" AND topic_id = $%d AND comment_id IS NULL", paramPos))
...
func (r *Repo) GetCounts(ctx context.Context, target vote.Target) (*vote.Counts, error) {
	...
	if target.CommentID == nil {
		...
		FROM votes
		WHERE topic_id = $1 AND comment_id IS NULL`
		...
		SELECT
			...
			COUNT(CASE WHEN reaction_type = -1 THEN 1 END) as downvotes
		FROM votes
		WHERE comment_id = $1 AND topic_id IS NULL`
